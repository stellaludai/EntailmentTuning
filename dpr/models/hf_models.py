...
def get_bert_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    ...
    question_encoder = HFBertEncoder.init_encoder(
        cfg.encoder.pretrained_model_path,
...
def get_bert_tensorizer(cfg):
    ...
    return BertTensorizer(tokenizer, sequence_length, pad_to_max=cfg.encoder.pad_to_max)
...
class HFBertEncoder(BertModel):
    ...
    @classmethod
    def init_encoder(
        cls, pretrained_model_path: str, projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, **kwargs
    ) -> BertModel:
        logger.info("Initializing HF BERT Encoder. model_name=%s", pretrained_model_path)
        ...
        if pretrained:
            return cls.from_pretrained(pretrained_model_path, config=cfg, project_dim=projection_dim, **kwargs)
        else:
