...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.best_validation_result_nll = None
        self.best_validation_result_rank = None

        self.best_cp_name = None
        self.best_cp_name_nll = None
        self.best_cp_name_rank = None
        self.cfg = cfg
        ...
        if self.cfg.local_rank in [-1, 0]:
            # wandb.login(key="09ee69fd4427532e99ee54366919e55c0d04393b")
            wandb.init(
                project="biencoder_training", 
                name="train_dpr_entail_tuned_snli_mnli_run_{0}".format(time.strftime("%m-%d-%H-%M-%S")))       
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        if not cfg.dev_datasets:
            ...
            '''
            validation_loss_rank = self.validate_average_rank()
            validation_loss_nll = self.validate_nll()

        if save_cp:
            cp_name = self._save_checkpoint(scheduler, epoch, iteration)
            logger.info("Saved checkpoint to %s", cp_name)

            if validation_loss_nll < (self.best_validation_result_nll or validation_loss_nll + 1):
                self.best_validation_result_nll = validation_loss_nll
                self.best_cp_name_nll = cp_name
                logger.info("New Best nll validation checkpoint %s", cp_name)
                
            if validation_loss_rank < (self.best_validation_result_rank or validation_loss_rank + 1):
                self.best_validation_result_rank = validation_loss_rank
                self.best_cp_name_rank = cp_name
                logger.info("New Best rank validation checkpoint %s", cp_name)
    ...
    def validate_nll(self) -> float:
        ...
        if cfg.local_rank in [-1, 0]:
            wandb.log({"validation_loss_nll": total_loss}, step=self.current_epoch)
        return total_loss
    ...
    def validate_average_rank(self) -> float:
        ...
        if cfg.local_rank in [-1, 0]:
            wandb.log({"validation_avg_rank": av_rank}, step=self.current_epoch)
        return av_rank
    ...
    def _train_epoch(
        ...
        dataset = 0
        if cfg.local_rank in [-1, 0]:
            pbar = tqdm(total=train_data_iterator.max_iterations)
        ...
        if cfg.local_rank in [-1, 0]:
            ...
            current_lr = scheduler.get_last_lr()[0]  # 假设使用的是PyTorch的scheduler
